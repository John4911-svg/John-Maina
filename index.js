...
myProfileBtn.addEventListener('click', myProfile);
myexperienceBtn.addEventListener('click', myExperience);
mySkillsBtn.addEventListener('click', mySkills);
myProjectBtn.addEventListener('click', myproject);
myContactBtn.addEventListener('click', myContact);
...
function updateButtonColors() {
  // 1. Get the current page from storage
const savedPage = localStorage.getItem('active');

  // 2. Reset ALL buttons to the default grey first
  const allButtons = [homeBtn, myProfileBtn, myexperienceBtn, myProjectBtn, myContactBtn,mySkillsBtn];
  allButtons.forEach(btn => {
    btn.style.backgroundColor = 'hsl(180, 10%, 59%)';
  });

  // 3. Highlight only the active one
  const activeColor = 'hsl(197, 100%, 40%)';
  
  if (savedPage === 'contact') {
    myContactBtn.style.backgroundColor = activeColor;
  } else if (savedPage === 'profile') {
    myProfileBtn.style.backgroundColor = activeColor;
  } else if (savedPage === 'experience') {
    myexperienceBtn.style.backgroundColor = activeColor;
  } else if (savedPage === 'skills') {
    mySkillsBtn.style.backgroundColor = activeColor;
  }else if (savedPage === 'project') {
    myProjectBtn.style.backgroundColor = activeColor;
  } else {
    homeBtn.style.backgroundColor = activeColor; // Default to Home
  }
}